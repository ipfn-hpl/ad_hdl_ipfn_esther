...
module phy_mac_link  #(
  ...
  always @(posedge ref_clk) begin
    if (reset_n == 1'b0) begin
      ...
      data_valid <= 1'b1;
    end else if ((mii_rx_dv_r0 == 1'b0) && (mii_rx_dv_r1 == 1'b0)) begin
      data_valid <= 1'b0;
    end
  end
  ...
  assign mii_rxd = (mii_rx_dv) ? ((clk_phase_res) ? mii_rxd_r1 : mii_rxd_r2) : 4'b0;
  assign clk_phase_res = clk_phase_r;
  assign sopack_w = sopack_r[0] & (!sopack_r[1]);
  assign eopack_w = eopack_r[0] & (!eopack_r[1]);
  assign data_valid_w = (clk_phase_res) ? data_valid : data_valid_d;
